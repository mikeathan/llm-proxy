...
7.  **Natural Task Completion**: The agent completes a task when it produces a final assistant message with substantive visible content (≥20 characters after stripping inline reasoning blocks such as `<think>`/`<reasoning>`) and no further tool calls. Completion requires at least one tool result anywhere in the run's history — the model must have actually done work, not just answered from first-turn text. Reasoning-only interleaves between tool results and the final answer do not block completion; the immediately-preceding message is not required to be a tool result. The synthetic `submit_final_answer` tool is removed (commit `f89b2cf`, PR #27). Heuristic keyword matching ("task complete", "summary") is not used. When the model returns empty after tools, the agent issues a single nag ("continue with the task") rather than looping indefinitely; if the response is still empty, a fallback answer (saved content-with-tools or best-available assistant text) is used.
