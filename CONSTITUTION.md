...
5.  **Token Budgeting & Dynamic Sieve (Open Claw v3)**: To prevent context size overflow, the system enforces a strict character-based history budget (default 15,000 characters).
    *   **Locked Head**: The System Prompt and the User's Initial Task are immutable and never pruned.
    *   **Iterative Middle-Pruning**: When the budget is exceeded, history is distilled from the middle. If the payload is still too large, the Priority Tail is iteratively reduced (down to a minimum of 2 messages) to guarantee the budget is honored.
    *   **Heuristic & Greedy Awareness**: If you have already provided a comprehensive Markdown report but are unable to emit the JSON tool call due to technical constraints (token limits, cutoffs), the system will attempt to recover your intent or accept your text as a final submission. However, you should always strive to use the formal `submit_final_answer` tool as your primary exit point.
