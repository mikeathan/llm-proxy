...
7.  **Explicit Task Completion**: The `submit_final_answer` tool is the canonical path to task completion in automation mode. In chat mode, the agent exits after a tool-result→assistant-reply cycle or after detecting premature termination (empty/repeated output). Heuristic keyword matching ("task complete", "summary") is not used. Step-level progress is tracked via the `complete_step` tool — the agent calls it to mark the current `[ACTIVE]` step as `[DONE]` and advance to the next `[PENDING]` step. The execution state (`PlanState`) is rendered as a compact `[DONE]`/`[ACTIVE]`/`[PENDING]` block pinned at index [1] of the conversation history, surviving sieve truncation.
...
14. **Execution State Tracking**: Multi-step automation tasks require a deterministic progress tracker that survives conversation history truncation.
      *   **PlanState**: A `PlanState` struct (`models/state.go`) holds a goal string and an ordered list of steps, each with a `StepStatus`: `PENDING`, `ACTIVE`, or `DONE`.
      *   **State Block**: At the start of every turn, the state is rendered as a compact `[DONE]`/`[ACTIVE]`/`[PENDING]` block and injected as a system message at index [1] of the prompt (right after the base system prompt). This ensures the model always knows which steps are completed, even after the physical sieve truncates history.
      *   **Step Transitions**: The agent calls `complete_step(notes)` to mark the current `[ACTIVE]` step done and advance to the next `[PENDING]` step. The `complete_step` tool call and its result enter the standard conversation history — the state block at [1] maintains ground truth regardless of history length.
15. **Tool Result LRU Cache**: Identical terminal commands produce identical output within a session, but re-executing them wastes time and context budget.
      *   **Cache**: A thread-safe LRU cache (`TokenCache` in `internal/core/assistant/tool_cache.go`, backed by `container/list` and `sync.RWMutex`) stores terminal command results keyed by the command argument string. Capacity is capped at 500 entries with oldest-entry eviction.
      *   **Interception**: Before every `execute_terminal_command` call, the cache is checked for an exact match. If found, the cached result is injected synthetically and the tool is never executed. This has zero false positives because it's exact match only.
      *   **Invalidation**: After any successful file mutation tool (`write_file`, `edit_file`, `append_file`), the entire cache is flushed to prevent stale reads (`cat` returning old data after a `write_file`).
