...
func TestMaybeNudge_NoGuards(t *testing.T) {
	...
	if s.stopGuard.attempts != 0 {
		t.Errorf("expected stopGuardAttempts untouched, got %d", s.stopGuard.attempts)
	}
	if s.finalize.finalizeAttempts != 0 {
		t.Errorf("finalizeAttempts must stay untouched by maybeNudge, got %d", s.finalize.finalizeAttempts)
	}
}

func TestMaybeNudge_FirstNudge(t *testing.T) {
	s := newTestSession()
	s.stopGuard.guards = []StopGuard{&mockStopGuard{nudge: "review your work"}}
	...
	if s.stopGuard.attempts != 1 {
		t.Errorf("expected stopGuardAttempts 1, got %d", s.stopGuard.attempts)
	}
}

func TestMaybeNudge_BoundedByCap(t *testing.T) {
	s := newTestSession()
	s.stopGuard.guards = []StopGuard{&mockStopGuard{nudge: "nudge"}}
	...
	if s.stopGuard.attempts != maxStopGuardAttempts {
		t.Errorf("expected stopGuardAttempts capped at %d, got %d", maxStopGuardAttempts, s.stopGuard.attempts)
	}
}

func TestMaybeNudge_GuardAllows(t *testing.T) {
	s := newTestSession()
	s.stopGuard.guards = []StopGuard{&mockStopGuard{nudge: ""}}
	if nudge, ok := s.maybeNudge(); ok {
		t.Fatalf("expected no nudge when the guard allows, got %q", nudge)
	}
	if s.stopGuard.attempts != 0 {
		t.Errorf("stopGuardAttempts must not increment when the guard allows, got %d", s.stopGuard.attempts)
	}
}

func TestMaybeNudge_GuardErrorAllows(t *testing.T) {
	s := newTestSession()
	s.stopGuard.guards = []StopGuard{&mockStopGuard{err: errors.New("boom")}}
	if nudge, ok := s.maybeNudge(); ok {
		t.Fatalf("expected finalization allowed on guard error, got %q", nudge)
	}
	if s.stopGuard.attempts != 0 {
		t.Errorf("stopGuardAttempts must not increment on guard error, got %d", s.stopGuard.attempts)
	}
}
