...
// TestFilterStreamingMarkup covers stream.go's FilterStreamingMarkup: only
// real markup delimiters truncate; prose less-than/braces/quoted-type do not.
...
// TestNormalizeContent covers stream.go's normalizeContent (empty text-block
// glitch stripping and text extraction).
