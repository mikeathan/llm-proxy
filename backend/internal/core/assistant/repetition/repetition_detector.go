...
func (rd *Detector) Check(logger logging.Logger, toolCalls []proxy.ToolCall) (bool, string, error) {
	for _, tc := range toolCalls {
		...
		if isSpiral, err := rd.trackConsecutive(key.name, key.args); isSpiral {
			return true, "", err
		}
...
// trackConsecutive updates the consecutive-same-tool streak and reports
// whether it has become a spiral: 12+ consecutive calls to one tool that
// recycle a small set of argument values (no exploration). Varied-argument
// bursts are legitimate exploration/batching, not a spiral.
func (rd *Detector) trackConsecutive(name, args string) (bool, error) {
	if name != rd.lastTool {
		rd.consecutiveToolStreak = 0
		rd.lastTool = name
		rd.streakArgs = map[string]struct{}{args: {}}
		return false, nil
	}
	...
	rd.recentCalls = nil
	rd.streakArgs = nil
	return true, fmt.Errorf(errFmtSpiral, name, SpiralStreakThreshold)
}

// feedWindows appends a call to the bounded oscillation and cycle windows.
func (rd *Detector) feedWindows(key toolKey) {
...
func (rd *Detector) CheckAlternating() (bool, error) {
...
func (rd *Detector) CheckSequenceRepeat() (bool, error) {
	if len(rd.seqWindow) < nGramWindowSize {
		return false, nil
	}
...
func (rd *Detector) CheckSameTarget(toolCalls []proxy.ToolCall) (bool, error) {
	if len(toolCalls) == 0 {
		return false, nil
	}
