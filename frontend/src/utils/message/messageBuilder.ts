...
import { MODEL_BUSY_NOTICE, MODEL_STARTING_NOTICE, TRANSPORT_ERROR_LABEL, UPSTREAM_RETRYING_TEMPLATE } from '../../constants/labels'
...
function upstreamRetryMessage(p: UpstreamEventPayload): string {
  ...
  if (p?.reason === 'model_busy') {
    ...
    // server's explanation is already a complete, human sentence — show it as-is
    // rather than wrapping it in a second "model in use" prefix.
    return p.error || MODEL_BUSY_NOTICE
  }
