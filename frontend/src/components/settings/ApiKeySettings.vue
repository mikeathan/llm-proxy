...
// ─────────────────────────────────────────────────────────────
// Selection – tracked by stable ID string.
// The panel is shown/hidden exclusively by `selectedId !== null`.
// Edit buffers are decoupled from props once populated.
// ─────────────────────────────────────────────────────────────
...
<template>
  <div class="api-key-manager">
    ...
    <div class="keys-list">
      ...
      >
        <span class="key-dot" :class="{ 'key-dot--active': selectedId === item.id }"></span>
        <div class="key-info">
    ...
    <div v-show="selectedId !== null" class="edit-panel">
      <div class="edit-panel-header">
        ...
        <button type="button" class="btn-close" @click.stop="closePanel" title="Close">✕</button>
      </div>
      ...
      <div class="edit-panel-body">
        ...
        <div class="field">
          ...
          <input v-model="editName" type="text" class="form-input" placeholder="e.g. Personal" autocomplete="off">
        </div>
        ...
        <div class="field">
          ...
          <div class="input-row">
            ...
            >
            <button type="button" class="btn-icon" @click.stop="showValue = !showValue">
              {{ showValue ? '🙈' : '👁️' }}
            </button>
          </div>
        </div>
        ...
        <!-- Test feedback -->
        <div v-if="testLoading || testSuccess || testError" class="test-feedback">
          <div v-if="testLoading"  class="test-loading"><span class="spinner"></span> Verifying…</div>
          <div v-else-if="testSuccess" class="test-success">✓ {{ testSuccess }}</div>
        ...
        <div v-if="confirmRemove" class="confirm-remove">
          <span class="confirm-text">Remove "{{ editName }}"?</span>
          <div class="confirm-actions">
            <button type="button" class="btn-cancel"  @click.stop="confirmRemove = false">Cancel</button>
            <button type="button" class="btn-confirm" @click.stop="confirmAndRemove">Yes, remove</button>
          </div>
        </div>

        <!-- Action bar -->
        <div class="panel-actions">
          <button type="button" class="btn-test" :disabled="testLoading" @click.stop="testSelected">
            <span v-if="testLoading" class="spinner-sm"></span>
            <span v-else>🧪</span>
            Test Connection
          </button>
          <div class="action-row">
            <button type="button" class="btn-danger" @click.stop="confirmRemove = true">🗑 Remove</button>
            <button type="button" class="btn-save"   @click.stop="saveEdit">Save Changes</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="add-section">
      ...
      <div class="add-body">
        <input v-model="newKeyName"  type="text"     class="form-input" placeholder="Key name (e.g. Personal)" autocomplete="off">
        <div class="input-row">
          <input v-model="newKeyValue" type="password" class="form-input" placeholder="API Key value" autocomplete="new-password">
          <button type="button" class="btn-add" :disabled="!newKeyValue" @click.stop="addKey">Add</button>
        </div>
      </div>
    </div>
...
/* ── Typography & inputs ── */
.form-label  { @apply block text-sm font-semibold text-gray-200; }
.form-helper { @apply text-xs text-gray-500 mb-3; }
.form-input  {
...
.input-row { @apply flex gap-2; }
.input-row .form-input { @apply flex-1; }
...
.key-item--selected { @apply bg-blue-600/10; }
.key-dot            { @apply w-2 h-2 rounded-full bg-gray-600 shrink-0 transition-colors; }
.key-dot--active    { @apply bg-blue-400; }
.key-info           { @apply flex-1 min-w-0; }
.key-name           { @apply text-xs font-bold text-gray-200 truncate; }
.key-preview        { @apply text-[10px] text-gray-500 font-mono; }
.key-badge {
  @apply text-[10px] font-bold text-blue-400 bg-blue-600/15
         border border-blue-600/30 px-2 py-0.5 rounded-full shrink-0;
}
.empty-keys { @apply p-4 text-center text-xs text-gray-500 italic; }
...
.edit-panel-title { @apply text-xs font-bold text-blue-300 tracking-wide flex-1 min-w-0 truncate; }
.btn-close {
  @apply text-gray-400 hover:text-white text-xs px-1.5 py-0.5 rounded transition-colors shrink-0 ml-2;
}
.edit-panel-body { @apply p-4 space-y-3; }
.field            { @apply space-y-1; }
.field-label      { @apply block text-xs font-semibold text-gray-400; }
...
.confirm-text    { @apply text-xs text-red-300 font-medium flex-1 min-w-0 truncate; }
.confirm-actions { @apply flex gap-2 shrink-0; }
.btn-cancel  {
...
/* ── Test feedback ── */
.test-feedback { @apply text-xs rounded-md; }
.test-loading  { @apply text-blue-400 flex items-center gap-2 px-2 py-1; }
.test-success  { @apply text-green-400 bg-green-950/20 border border-green-500/30 px-3 py-2 rounded; }
.test-error    { @apply text-red-400   bg-red-950/20   border border-red-500/30   px-3 py-2 rounded break-words; }

/* ── Spinners ── */
.spinner    { @apply inline-block w-3 h-3 border-2 border-blue-400 border-t-transparent rounded-full animate-spin; }
.spinner-sm { @apply inline-block w-3 h-3 border border-current border-t-transparent rounded-full animate-spin; }
</style>
